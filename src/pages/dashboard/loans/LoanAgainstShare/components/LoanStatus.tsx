...
const STATUS_STEPS = [
  ...
  { key:"Verification", icon:"📄", desc:"Documents & holdings being verified" },
  ...
  if (!isSubmitted) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-sm" style={{color:C.gray}}>Submit your loan against share application to track its status here.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          {/* Share details */}
          <div className="bg-white rounded-2xl p-6 shadow-sm" style={{border:`1px solid ${C.teal}18`}}>
            <p className="text-xs font-bold uppercase tracking-wide mb-4" style={{color:C.teal}}>Share Details</p>
            <div className="space-y-3">
              {[
                ["Company",          submittedApp?.shareCompanyName??"—"],
                ["Value / Share",    `₹${(submittedApp?.valueOfOneShare??0).toLocaleString("en-IN")}`],
                ["Quantity",         String(submittedApp?.quantityOfShare??0)],
                ["Total Share Value",`₹${(submittedApp?.totalShareValue??0).toLocaleString("en-IN")}`],
              ].map(([l,v])=>(
