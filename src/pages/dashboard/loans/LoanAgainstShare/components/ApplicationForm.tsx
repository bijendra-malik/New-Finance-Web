...
interface ApplicationFormProps {
  userName?: string;
  userEmail?: string;
  onSubmit?: (id: string, app: LoanAgainstShareApplication) => void;
}
interface FormData {
  fullName:string; mobile:string; email:string; dob:string; panNumber:string;
  state:string; city:string; pincode:string; residenceStatus:string; residenceStatusOther:string;
  shareCompanyName:string; valueOfOneShare:number; quantityOfShare:number;
  employmentType:string;
...
const ApplicationForm = ({userName="",userEmail="",onSubmit}:ApplicationFormProps) => {
  ...
  const [form, setForm] = useState<FormData>({
    ...
    shareCompanyName:"", valueOfOneShare:0, quantityOfShare:1,
    employmentType:"",
  ...
  useEffect(() => {
    let cancelled = false;
    getEmploymentTypesFor("loan-against-share", "loanAgainstShareEmploymentTypes")
  ...
  const employmentTypeOptions = useMemo(
    () => (employmentTypesState.length > 0 ? employmentTypesState : [...masters.loanAgainstShareEmploymentTypes]),
    [employmentTypesState, masters.loanAgainstShareEmploymentTypes]
  );
...
const hasExposure = parseInt(form.existingEMI) > 0 || parseInt(form.existingLoanAmount) > 0;
  ...
  const totalShareValue = useMemo(
    () => form.valueOfOneShare * form.quantityOfShare,
    [form.valueOfOneShare, form.quantityOfShare]
  );
...
// ── Shared validation constants (used by computeErrors below) ──
  ...
  const computeErrors = (draft:FormData = form) => {
    ...
    if(!draft.quantityOfShare) e.quantityOfShare="Quantity of share is required";
  ...
  const handleSubmit = async (e:React.FormEvent) => {
    ...
    const app: LoanAgainstShareApplication = {
      _id: `LAS${Date.now()}`,
    ...
    <div className="max-w-3xl mx-auto space-y-5">
      ...
      sections={buildSuccessSections(submittedApp, {
        productSection: {
          ...
          rows: [
            { label: "Share Company", value: submittedApp.shareCompanyName },
            { label: "Value of One Share", value: submittedApp.valueOfOneShare ? `₹${submittedApp.valueOfOneShare.toLocaleString("en-IN")}` : undefined },
            { label: "Quantity", value: submittedApp.quantityOfShare ? submittedApp.quantityOfShare.toLocaleString("en-IN") : undefined },
            { label: "Total Portfolio Value", value: submittedApp.totalShareValue ? `₹${submittedApp.totalShareValue.toLocaleString("en-IN")}` : undefined },
          ],
        },
      })}
    />
    ...
    <form className="max-w-4xl mx-auto" onSubmit={handleSubmit} noValidate>
      ...
      <div className="mb-6">
        <h1 className="text-xl font-bold" style={{color:C.dark}}>
          Unlock the best Loan Against Share offers suitable for your needs from 43+ lenders
        </h1>
      ...
      <FormCard title="Loan Requirements" subtitle="How much do you need and for how long?">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div id="valueOfOneShare"><FieldLabel label="Value of one share" required/>
            <TextField type="number" value={form.valueOfOneShare===0?"":String(form.valueOfOneShare)}
              onChange={v=>set("valueOfOneShare",Math.max(0,parseInt(v)||0))} placeholder="e.g. 2500" err={errors.valueOfOneShare}/>
          </div>
          <div id="quantityOfShare"><FieldLabel label="Quantity of share" required/>
            <TextField type="number" value={form.quantityOfShare===0?"":String(form.quantityOfShare)}
              onChange={v=>set("quantityOfShare",Math.max(0,parseInt(v)||0))} placeholder="e.g. 500" err={errors.quantityOfShare}/>
          </div>
          <div id="totalShareValue"><FieldLabel label="Total Share Value"/>
            <TextField value={totalShareValue===0?"":totalShareValue.toLocaleString("en-IN")} onChange={()=>{}} placeholder="Total Market Value" disabled/>
          </div>
        </div>
      </FormCard>
            ...
            <div id="gstNumber"><FieldLabel label="GST No (if available)"/>
              ...
              <div id="businessEstablishedDate"><FieldLabel label="Date Of Business Establishment" required/>
                <DateField value={form.businessEstablishedDate} onChange={v=>set("businessEstablishedDate",v)}
                  ...
                  portalId="las-business-established-datepicker-portal"/>
              </div>
