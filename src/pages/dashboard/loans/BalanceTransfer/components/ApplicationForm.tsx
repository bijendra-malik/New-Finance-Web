...
interface ApplicationFormProps {
  userName?: string;
  userEmail?: string;
  onSubmit?: (id: string, app: BalanceTransferApplication) => void;
}
interface FormData {
  fullName:string; mobile:string; email:string; dob:string; panNumber:string;
  state:string; city:string; pincode:string; residenceStatus:string; residenceStatusOther:string;
  balanceTransferType:string; balanceTransferTypeOther:string; currentPropertyValue:string;
  loanAmount:number; loanTenureYears:number; loanTenureYearsCustom:number; topUpAmount:string;
  employmentType:string;
...
const ApplicationForm = ({userName="",userEmail="",onSubmit}:ApplicationFormProps) => {
  ...
  const [form, setForm] = useState<FormData>({
    ...
    balanceTransferType:"", balanceTransferTypeOther:"", currentPropertyValue:"",
    loanAmount:0, loanTenureYears:0, loanTenureYearsCustom:0, topUpAmount:"",
    employmentType:"",
  ...
  useEffect(() => {
    let cancelled = false;
    getEmploymentTypesFor("balance-transfer", "homeEmploymentTypes")
...
const hasExposure = parseInt(form.existingEMI) > 0 || parseInt(form.existingLoanAmount) > 0;
  ...
  const computeErrors = (draft:FormData = form) => {
    ...
    if(!draft.balanceTransferType) e.balanceTransferType="Type of balance transfer is required";
    else if(draft.balanceTransferType===OTHER_OPTION&&!draft.balanceTransferTypeOther.trim()) e.balanceTransferTypeOther="Please mention type of balance transfer";
    ...
    else if(draft.loanTenureYears===MORE_THAN_TENURE_OPTION&&(form.loanTenureYearsCustom<11||form.loanTenureYearsCustom>30)) e.loanTenureYearsCustom="Custom tenure must be between 11 and 30 years";
  ...
  const handleSubmit = async (e:React.FormEvent) => {
    ...
    const app: BalanceTransferApplication = {
      _id: `BT${Date.now()}`,
      ...
      balanceTransferType:form.balanceTransferType===OTHER_OPTION?form.balanceTransferTypeOther:form.balanceTransferType,
      currentPropertyValue:form.currentPropertyValue.trim()?parseInt(form.currentPropertyValue):undefined,
      topUpAmount:form.topUpAmount.trim()?parseInt(form.topUpAmount):undefined,
  ...
  if(submitted && submittedApp && !showFormAfterSubmit) return (
    <div className="max-w-3xl mx-auto space-y-5">
      ...
      sections={buildSuccessSections(submittedApp, {
        extraLoanRows: [
          { label: "Transfer Type", value: submittedApp.balanceTransferType },
          { label: "Current Property Value", value: submittedApp.currentPropertyValue ? `₹${submittedApp.currentPropertyValue.toLocaleString("en-IN")}` : undefined },
          { label: "Top-up Amount", value: submittedApp.topUpAmount ? `₹${submittedApp.topUpAmount.toLocaleString("en-IN")}` : undefined },
        ],
      })}
    />
    </div>
    ...
    <form className="max-w-4xl mx-auto" onSubmit={handleSubmit} noValidate>
      ...
      <div className="mb-6">
        <h1 className="text-xl font-bold" style={{color:C.dark}}>
          Unlock the best Balance Transfer offers suitable for your needs from 43+ lenders
        </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <SelectWithOther
            ...
            value={form.balanceTransferType} onChange={v=>{
              set("balanceTransferType",v);
              if(v!==OTHER_OPTION) set("balanceTransferTypeOther","");
            }} options={masters.existingLoanTypes} err={errors.balanceTransferType}
            otherId="balanceTransferTypeOther" otherLabel="Mention Type of Balance Transfer"
            otherValue={form.balanceTransferTypeOther} onOtherChange={v=>set("balanceTransferTypeOther",v)}
            ...
            <div id="gstNumber"><FieldLabel label="GST No (if available)"/>
              ...
              <div id="businessEstablishedDate"><FieldLabel label="Date Of Business Establishment" required/>
                <DateField value={form.businessEstablishedDate} onChange={v=>set("businessEstablishedDate",v)}
                  ...
                  portalId="balance-transfer-business-established-datepicker-portal"/>
              </div>
