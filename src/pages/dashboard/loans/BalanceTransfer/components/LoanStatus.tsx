  ...
  if (!isSubmitted) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-sm" style={{color:C.gray}}>Submit your balance transfer application to track its status here.</p>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm" style={{border:`1px solid ${C.navy}18`}}>
            <p className="text-xs font-bold uppercase tracking-wide mb-4" style={{color:C.navy}}>Transfer Details</p>
            <div className="grid grid-cols-2 gap-3">
              {[
                ["Transfer Amount", `₹${(submittedApp?.loanAmount??0).toLocaleString("en-IN")}`],
          ...
          {/* Existing loans */}
          {((submittedApp?.existingLoanTypes?.length??0) > 0 || (submittedApp?.existingLoanAmount??0) > 0) && (
            <div className="bg-white rounded-2xl p-6 shadow-sm" style={{border:`1px solid #e2e8f0`}}>
              ...
              {(submittedApp?.existingLoanTypes?.length??0)>0&&(
                <div className="mt-3">
                  <p className="text-[10px] font-bold uppercase tracking-wide mb-2" style={{color:C.gray}}>Loan Types</p>
                  <div className="flex flex-wrap gap-1.5">
                    {submittedApp?.existingLoanTypes.map(t=>(
                      <span key={t} className="px-2 py-0.5 rounded-full text-xs font-semibold"
                        style={{background:C.tealBg,color:C.teal,border:`1px solid ${C.teal}33`}}>{t}</span>
                    ))}
                  </div>
                </div>
              )}
