...
const COMPANY_EVALUATION = "Company Valuation";

interface ApplicationFormProps {
  userName?: string;
  userEmail?: string;
  onSubmit?: (id: string, app: FDIApplication) => void;
}
interface FormData {
  fullName:string; mobile:string; email:string; dob:string; panNumber:string;
...
const ApplicationForm = ({userName="",userEmail="",onSubmit}:ApplicationFormProps) => {
  ...
  const [submittedApp, setSubmittedApp] = useState<FDIApplication|null>(null);
  ...
  const [form, setForm] = useState<FormData>({
    ...
    companyEvaluationValue:0, interestedInEquityPartner:"", equityShareOffered:0,
    employmentType:"",
  ...
  const computeErrors = (draft:FormData = form) => {
    ...
    if(draft.collateralPropertyType===COMPANY_EVALUATION){
      if(!draft.companyEvaluationValue) e.companyEvaluationValue="Current valuation of the company is required";
      if(!draft.interestedInEquityPartner) e.interestedInEquityPartner="Please select Yes or No";
      else if(draft.interestedInEquityPartner==="Yes"){
        if(!draft.equityShareOffered) e.equityShareOffered="Equity share is required";
        else if(draft.equityShareOffered>100) e.equityShareOffered="Equity share cannot exceed 100%";
      }
    }
  ...
  const handleSubmit = async (e:React.FormEvent) => {
    ...
    const app: FDIApplication = {
      ...
      companyEvaluationValue:form.collateralPropertyType===COMPANY_EVALUATION?form.companyEvaluationValue:undefined,
      interestedInEquityPartner:form.collateralPropertyType===COMPANY_EVALUATION?form.interestedInEquityPartner:undefined,
      equityShareOffered:form.collateralPropertyType===COMPANY_EVALUATION&&form.interestedInEquityPartner==="Yes"?form.equityShareOffered:undefined,
      ...
      <div className="rounded-3xl overflow-hidden shadow-xl" style={{border:`1px solid ${C.teal}33`}}>
        ...
        <div className="p-10 text-center bg-white">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-left mb-6">
            {[
              ["Fund Amount",  `₹${submittedApp.loanAmount.toLocaleString("en-IN")}`],
              ["Tenure",       `${submittedApp.loanTenure} months`],
              ["Fund Against", submittedApp.collateralPropertyType||"—"],
              ...(submittedApp.collateralPropertyType===COMPANY_EVALUATION
                ? [["Equity Partner", submittedApp.interestedInEquityPartner||"—"]]
                : []),
    ...
    <form className="max-w-4xl mx-auto" onSubmit={handleSubmit} noValidate>
      <div className="mb-6">
        <h1 className="text-xl font-bold" style={{color:C.dark}}>
          Unlock the best FDI offers suitable for your needs from 43+ lenders
        </h1>
      ...
      <FormCard title="Funds Requirements" subtitle="How much do you need and for how long?">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div id="loanAmount"><FieldLabel label="Required Fund Amount" required/>
            <TextField type="number" value={form.loanAmount===0?"":String(form.loanAmount)}
              onChange={v=>set("loanAmount",Math.max(0,parseInt(v)||0))} placeholder="Min ₹100 Cr, e.g. 1000000000" err={errors.loanAmount}/>
          </div>
          <TenureYearsField
            ...
            options={masters.fdiTenureYears} err={errors.loanTenureYears}
          />
          <SelectWithOther
            ...
            }} options={masters.fdiFundAgainstTypes} err={errors.collateralPropertyType}
            otherId="collateralPropertyTypeOther" otherLabel="Mention Fund Against Type"
            otherValue={form.collateralPropertyTypeOther} onOtherChange={v=>set("collateralPropertyTypeOther",v)}
            otherPlaceholder="Enter fund against type" otherErr={errors.collateralPropertyTypeOther}
          />

          {form.collateralPropertyType===COMPANY_EVALUATION&&(<>
            <div id="companyEvaluationValue"><FieldLabel label="Current Valuation of the Company" required/>
              <TextField type="number" value={form.companyEvaluationValue===0?"":String(form.companyEvaluationValue)}
                onChange={v=>set("companyEvaluationValue",Math.max(0,parseInt(v)||0))} placeholder="e.g. 5000000" err={errors.companyEvaluationValue}/>
            </div>
            <div id="interestedInEquityPartner"><FieldLabel label="Interested to Take Investor as Equity Partner" required/>
              <SelectField value={form.interestedInEquityPartner} onChange={v=>{
                set("interestedInEquityPartner",v);
                if(v!=="Yes") set("equityShareOffered",0);
              }} options={masters.yesNo} placeholder="Select" err={errors.interestedInEquityPartner}/>
            </div>
            {form.interestedInEquityPartner==="Yes"&&(
              <div id="equityShareOffered"><FieldLabel label="What Equity Share Would You Like to Offer the Investor?" required/>
                <TextField type="number" value={form.equityShareOffered===0?"":String(form.equityShareOffered)}
                  onChange={v=>set("equityShareOffered",Math.min(100,Math.max(0,parseInt(v)||0)))} placeholder="e.g. 10 (max 100%)" err={errors.equityShareOffered}/>
              </div>
            )}
          </>)}

          <div id="collateralPropertyMarketValue"><FieldLabel label="Collateral Property Market Value (Approx)" required={collateralRequired}/>
          ...
          <div id="collateralPropertyAge"><FieldLabel label="Collateral Property Age (in years)" required={collateralRequired}/>
          ...
          <div id="collateralPropertyState"><FieldLabel label="Collateral Property State" required={collateralRequired}/>
          ...
          <div id="collateralPropertyCity"><FieldLabel label="Collateral Property City" required={collateralRequired}/>
          ...
          <PincodeSelectField
            id="collateralPropertyPincode" label="Collateral Property Pincode" required={collateralRequired}
