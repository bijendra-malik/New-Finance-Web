...
export interface DashboardShellProps<T> {
  ...
  renderForm: (ctx: { userName: string; userEmail: string; onSubmit: (id: string, app: T) => void }) => ReactNode;
  renderStatus: (ctx: { applicationId: string; isSubmitted: boolean; submittedApp: T | null }) => ReactNode;
}

const DashboardShell = <T,>({
  productName,
  icon,
  stats,
  applicationDesc,
  ...
  const [submittedApp,  setSubmittedApp]  = useState<T | null>(null);
  ...
  const tabs: typeof TABS = [
    { ...TABS[0], desc: applicationDesc ?? TABS[0].desc },
    TABS[1],
  ];
    ...
    <div className="min-h-screen flex" style={{ background: "#f0f4f8", marginTop: "100px" }}>
      ...
      >
        ...
        <div className="px-5 py-5 border-b" style={{ borderColor:"rgba(255,255,255,0.08)" }}>
          <div className="flex items-center gap-3 cursor-pointer group" onClick={() => navigate("/")}>
            ...
            <div>
              <p className="font-extrabold text-white text-sm leading-tight">{productName}</p>
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <div className="px-4 md:px-8 py-6 md:py-10">
          ...
          {/* The form stays mounted (hidden) so a submitted application survives tab switches */}
          <div style={activeTab === "application" ? undefined : { display: "none" }}>
            {renderForm({ userName: user.name, userEmail: user.email, onSubmit: handleApplicationSubmit })}
          </div>
          {activeTab === "status" && (
