  ...
  if (!isSubmitted) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-sm" style={{color:C.gray}}>Submit your lease rental discounting application to track its status here.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm" style={{border:`1px solid ${C.navy}18`}}>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                ["Monthly Lease Income", `₹${(submittedApp?.monthlyLeaseIncome??0).toLocaleString("en-IN")}`],
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm" style={{border:`1px solid ${C.teal}18`}}>
            <p className="text-xs font-bold uppercase tracking-wide mb-4" style={{color:C.teal}}>Lease Property Details</p>
            <div className="space-y-3">
              {[
                ["Market Value", `₹${(submittedApp?.leasePropertyMarketValue??0).toLocaleString("en-IN")}`],
                ["Duration",     `${submittedApp?.leasePropertyDuration??0} years`],
                ["Age",          `${submittedApp?.leasePropertyAge??0} years`],
                ["City",         `${submittedApp?.leasePropertyCity??""}, ${submittedApp?.leasePropertyState??""}`],
                ["Pincode",      submittedApp?.leasePropertyPincode??"—"],
              ].map(([l,v])=>(
