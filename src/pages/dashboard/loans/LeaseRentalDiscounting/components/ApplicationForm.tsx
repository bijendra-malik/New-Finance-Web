...
interface FormData {
  ...
  leasePropertyState:string; leasePropertyCity:string; leasePropertyPincode:string; leasePropertyPincodeOther:string;
  employmentType:string;
...
const ApplicationForm = ({userName="",userEmail="",onSubmit}:ApplicationFormProps) => {
  ...
  const [submittedApp, setSubmittedApp] = useState<LeaseRentalApplication|null>(null);
  ...
  const [form, setForm] = useState<FormData>({
    ...
    leasePropertyState:"", leasePropertyCity:"", leasePropertyPincode:"", leasePropertyPincodeOther:"",
    employmentType:"",
  ...
  const leasePropertyPincodeOptions = useMemo(
    () => form.leasePropertyState && form.leasePropertyCity ? masters.pincodesByLocation[pincodeLocationKey(form.leasePropertyState, form.leasePropertyCity)] ?? [] : [],
    [form.leasePropertyState, form.leasePropertyCity, masters.pincodesByLocation]
  );
  ...
  const computeErrors = (draft:FormData = form) => {
    ...
    else if(draft.leasePropertyPincode===OTHER_OPTION){
      if(!draft.leasePropertyPincodeOther.trim()) e.leasePropertyPincodeOther="Please mention pincode";
      else if(!/^\d{6}$/.test(draft.leasePropertyPincodeOther)) e.leasePropertyPincodeOther="Enter valid 6-digit pincode";
    }
  ...
  const handleSubmit = async (e:React.FormEvent) => {
    ...
    const app: LeaseRentalApplication = {
      _id: `LRD${Date.now()}`,
      ...
      leasePropertyPincode:form.leasePropertyPincode===OTHER_OPTION?form.leasePropertyPincodeOther:form.leasePropertyPincode,
        ...
        <div className="p-10 text-center bg-white">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-left mb-6">
            {[
              ...
              ["Lease Property City", submittedApp.leasePropertyCity],
              ["Lease Property State",submittedApp.leasePropertyState],
              ["Monthly Lease Income", `₹${submittedApp.monthlyLeaseIncome.toLocaleString("en-IN")}`],
      ...
      <FormCard title="Loan Requirements" subtitle="How much do you need and for how long?">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <TenureYearsField
            ...
            options={masters.leaseRentalTenureYears} err={errors.loanTenureYears}
          />
          <div id="monthlyLeaseIncome"><FieldLabel label="Monthly Income Through Lease"/>
            <TextField type="number" value={form.monthlyLeaseIncome} onChange={v=>set("monthlyLeaseIncome",v.replace(/\D/g,""))} placeholder="e.g. 80000"/>
          </div>
          <div id="totalLeaseAmount"><FieldLabel label="Total Amount To Be Received From Lease"/>
            <TextField type="number" value={form.totalLeaseAmount} onChange={v=>set("totalLeaseAmount",v.replace(/\D/g,""))} placeholder="e.g. 9600000"/>
          </div>
          ...
          <div id="leasePropertyMarketValue"><FieldLabel label="Lease Property Market Value (Approx)"/>
            <TextField type="number" value={form.leasePropertyMarketValue===0?"":String(form.leasePropertyMarketValue)}
              onChange={v=>set("leasePropertyMarketValue",Math.max(0,parseInt(v)||0))} placeholder="e.g. 6000000"/>
          </div>
          ...
          <PincodeSelectField
            ...
            options={leasePropertyPincodeOptions} cityReady={!!form.leasePropertyCity}
            value={form.leasePropertyPincode} onChange={v=>set("leasePropertyPincode",v)} err={errors.leasePropertyPincode}
            otherId="leasePropertyPincodeOther" otherValue={form.leasePropertyPincodeOther}
            onOtherChange={v=>set("leasePropertyPincodeOther",v)} otherErr={errors.leasePropertyPincodeOther}
          />
        </div>
      </FormCard>
      ...
      <FormCard title="Income Details" subtitle="Tell us about your business and income">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="md:col-span-2" id="employmentType"><FieldLabel label="Employment Type" required/>
            <SelectField value={form.employmentType} onChange={setEmploymentType} options={masters.leaseRentalEmploymentTypes} placeholder="Select" err={errors.employmentType}/>
          </div>
