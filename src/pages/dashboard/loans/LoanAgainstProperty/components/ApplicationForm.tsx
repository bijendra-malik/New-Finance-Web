...
interface ApplicationFormProps {
  userName?: string;
  userEmail?: string;
  onSubmit?: (id: string, app: LoanAgainstPropertyApplication) => void;
}
interface FormData {
  fullName:string; mobile:string; email:string; dob:string; panNumber:string;
...
const ApplicationForm = ({userName="",userEmail="",onSubmit}:ApplicationFormProps) => {
  ...
  const [submittedApp, setSubmittedApp] = useState<LoanAgainstPropertyApplication|null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    getEmploymentTypesFor("loan-against-property", "loanAgainstPropertyEmploymentTypes")
  ...
  const employmentTypeOptions = useMemo(
    () => (employmentTypesState.length > 0 ? employmentTypesState : [...masters.loanAgainstPropertyEmploymentTypes]),
    [employmentTypesState, masters.loanAgainstPropertyEmploymentTypes]
  );
...
// ── Shared validation constants (used by computeErrors below) ──
  ...
  const computeErrors = (draft:FormData = form) => {
    ...
    if(!draft.loanTenureYears) e.loanTenureYears="Select loan tenure (minimum 3 years)";
    else if(draft.loanTenureYears===MORE_THAN_TENURE_OPTION&&(form.loanTenureYearsCustom<11||form.loanTenureYearsCustom>20)) e.loanTenureYearsCustom="Custom tenure must be between 11 and 20 years";
  ...
  const handleSubmit = async (e:React.FormEvent) => {
    ...
    const app: LoanAgainstPropertyApplication = {
      _id: `LAP${Date.now()}`,
    ...
    <form className="max-w-4xl mx-auto" onSubmit={handleSubmit} noValidate>
      ...
      <div className="mb-6">
        <h1 className="text-xl font-bold" style={{color:C.dark}}>
          Unlock the best Loan Against Property offers suitable for your needs from 43+ lenders
        </h1>
      ...
      <FormCard title="Loan Requirements" subtitle="How much do you need and for how long?">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <TenureYearsField
            id="loanTenureYears" label="Required Loan Tenure (not less than 3 year)"
            ...
            options={masters.loanAgainstPropertyTenureYears} err={errors.loanTenureYears}
          />
          <SelectWithOther
            ...
            <div id="gstNumber"><FieldLabel label="GST No (if available)"/>
              ...
              <div id="businessEstablishedDate"><FieldLabel label="Date Of Business Establishment" required/>
                <DateField value={form.businessEstablishedDate} onChange={v=>set("businessEstablishedDate",v)}
                  ...
                  portalId="lap-business-established-datepicker-portal"/>
              </div>
