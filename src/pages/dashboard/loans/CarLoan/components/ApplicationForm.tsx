...
export interface CarLoanApplication {
  ...
  vehicleType?: string; transmissionType?: string; manufacturer?: string; model?: string; vehiclePurchaseType?: string;
...
interface ApplicationFormProps {
  userName?: string;
  userEmail?: string;
  onSubmit?: (id: string, app: CarLoanApplication) => void;
}
interface FormData {
  fullName:string; mobile:string; email:string; dob:string; panNumber:string;
...
const ApplicationForm = ({userName="",userEmail="",onSubmit}:ApplicationFormProps) => {
  ...
  useEffect(() => {
    let cancelled = false;
    getEmploymentTypesFor("car", "homeEmploymentTypes")
    ...
    <div className="max-w-3xl mx-auto space-y-5">
      ...
      sections={buildSuccessSections(submittedApp, {
        productSection: {
          ...
          rows: [
            { label: "Vehicle Type", value: submittedApp.vehicleType },
            { label: "Transmission", value: submittedApp.transmissionType },
            { label: "Manufacturer", value: submittedApp.manufacturer },
            { label: "Model", value: submittedApp.model },
            { label: "Purchase Type", value: submittedApp.vehiclePurchaseType },
          ],
        },
      })}
    />
    ...
    <form className="max-w-4xl mx-auto" onSubmit={handleSubmit} noValidate>
      ...
      <div className="mb-6">
        <h1 className="text-xl font-bold" style={{color:C.dark}}>
          Unlock the best Vehicle Loan offers suitable for your needs from 43+ lenders
        </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div id="loanAmount"><FieldLabel label="Required Loan Amount" required/>
            <TextField type="number" value={form.loanAmount===0?"":String(form.loanAmount)}
              onChange={v=>set("loanAmount",Math.max(0,parseInt(v)||0))} placeholder="e.g. 800000" err={errors.loanAmount}/>
          </div>
          <TenureYearsField
          ...
          <SelectWithOther
            ...
            }} options={masters.vehicleTypes} err={errors.vehicleType}
            otherId="vehicleTypeOther" otherLabel="Mention Vehicle Type"
            otherValue={form.vehicleTypeOther} onOtherChange={v=>set("vehicleTypeOther",v)}
          ...
          <SelectWithOther
            ...
            }} options={masters.transmissionTypes} err={errors.transmissionType}
            otherId="transmissionTypeOther" otherLabel="Mention Transmission Type"
          ...
          <SelectWithOther
            ...
            }} options={masters.vehiclePurchaseTypes} err={errors.vehiclePurchaseType}
            otherId="vehiclePurchaseTypeOther" otherLabel="Mention Purchase Type"
            otherValue={form.vehiclePurchaseTypeOther} onOtherChange={v=>set("vehiclePurchaseTypeOther",v)}
            otherPlaceholder="Enter purchase type" otherErr={errors.vehiclePurchaseTypeOther}
          />
        </div>
      </FormCard>
            ...
            <div id="gstNumber"><FieldLabel label="GST No (if available)"/>
              ...
              <div id="businessEstablishedDate"><FieldLabel label="Date Of Business Establishment" required/>
                <DateField value={form.businessEstablishedDate} onChange={v=>set("businessEstablishedDate",v)}
                  ...
                  portalId="carloan-business-established-datepicker-portal"/>
              </div>
