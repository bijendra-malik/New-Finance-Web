...
interface ApplicationFormProps {
  userName?: string;
  userEmail?: string;
  onSubmit?: (id: string, app: WorkingCapitalApplication) => void;
}
interface FormData {
  fullName:string; mobile:string; email:string; dob:string; panNumber:string;
...
const ApplicationForm = ({userName="",userEmail="",onSubmit}:ApplicationFormProps) => {
  ...
  const [submittedApp, setSubmittedApp] = useState<WorkingCapitalApplication|null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    getEmploymentTypesFor("working-capital", "workingCapitalEmploymentTypes")
  ...
  const employmentTypeOptions = useMemo(
    () => (employmentTypesState.length > 0 ? employmentTypesState : [...masters.workingCapitalEmploymentTypes]),
    [employmentTypesState, masters.workingCapitalEmploymentTypes]
  );
...
// ── Shared validation constants (used by computeErrors below) ──
  ...
  const handleSubmit = async (e:React.FormEvent) => {
    ...
    const app: WorkingCapitalApplication = {
      _id: `WC${Date.now()}`,
      ...
      <FormCard title="Loan Requirements" subtitle="How much do you need and for how long?">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <TenureYearsField
            ...
            options={masters.workingCapitalTenureYears} err={errors.loanTenureYears}
          />
          <SelectWithOther
