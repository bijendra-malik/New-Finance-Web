    ...
    >
      {app && (
        <>
          ...
          <StatusCard title="Collateral Property" accent="#26ae90">

          <div className="space-y-3">
            <div className="space-y-3">
              {([ ["Market Value", `₹${(app?.collateralPropertyMarketValue??0).toLocaleString("en-IN")}`], ["Age", `${app?.collateralPropertyAge??0} years`], ["City", `${app?.collateralPropertyCity??""}, ${app?.collateralPropertyState??""}`], ["Pincode", app?.collateralPropertyPincode??"—"], ]).map(([l, v]) => (
