...
interface ApplicationFormProps {
  userName?: string;
  userEmail?: string;
  onSubmit?: (id: string, app: GoldLoanApplication) => void;
}
interface FormData {
  fullName:string; mobile:string; email:string; dob:string; panNumber:string;
  state:string; city:string; pincode:string; residenceStatus:string; residenceStatusOther:string;
  typeOfLoan:string; typeOfLoanOther:string;
  goldCarats:string; goldCaratsOther:string; goldWeight:number;
  jewelryGoldWeight:number; jewelryStoneWeight:number;
  jewelryOtherMaterialName:string; jewelryOtherMaterialWeight:number;
  jewelryOtherMaterials:{name:string; weight:number}[];
  collateralPropertyMarketValue:number;
  employmentType:string;
...
const ApplicationForm = ({userName="",userEmail="",onSubmit}:ApplicationFormProps) => {
  ...
  const [submittedApp, setSubmittedApp] = useState<GoldLoanApplication|null>(null);
  ...
  const [form, setForm] = useState<FormData>({
    ...
    typeOfLoan:"", typeOfLoanOther:"",
    goldCarats:"", goldCaratsOther:"", goldWeight:0,
    jewelryGoldWeight:0, jewelryStoneWeight:0,
    jewelryOtherMaterialName:"", jewelryOtherMaterialWeight:0, jewelryOtherMaterials:[],
    collateralPropertyMarketValue:0,
    employmentType:"",
  ...
  const setTypeOfLoan = (v:string) => {
    setForm(p=>({
      ...p, typeOfLoan:v, typeOfLoanOther:v===OTHER_OPTION?p.typeOfLoanOther:"",
      goldCarats:"", goldCaratsOther:"", goldWeight:0,
      jewelryGoldWeight:0, jewelryStoneWeight:0,
      jewelryOtherMaterialName:"", jewelryOtherMaterialWeight:0, jewelryOtherMaterials:[],
    }));
    setTouched(p=>(p.typeOfLoan?p:{...p,typeOfLoan:true}));
  };

  const addJewelryOtherMaterial = () => {
    const name = form.jewelryOtherMaterialName.trim();
    if(!name || !form.jewelryOtherMaterialWeight) return;
    setForm(p=>({
      ...p,
      jewelryOtherMaterials:[...p.jewelryOtherMaterials, {name, weight:p.jewelryOtherMaterialWeight}],
      jewelryOtherMaterialName:"", jewelryOtherMaterialWeight:0,
    }));
  };
  const removeJewelryOtherMaterial = (idx:number) =>
    setForm(p=>({...p, jewelryOtherMaterials:p.jewelryOtherMaterials.filter((_,i)=>i!==idx)}));

  const setGoldCarats = (v:string) => {
    set("goldCarats",v);
    if(v!==OTHER_OPTION) set("goldCaratsOther","");
  };
  ...
  const computeErrors = (draft:FormData = form) => {
    ...
    if(!draft.goldCarats) e.goldCarats="Please select gold Karat";
    else if(draft.goldCarats===OTHER_OPTION&&!draft.goldCaratsOther.trim()) e.goldCaratsOther="Please mention gold Karat";
    if(draft.typeOfLoan!=="Jewellery"&&!draft.goldWeight) e.goldWeight="Gold weight is required";
    if(draft.typeOfLoan==="Jewellery"&&!draft.jewelryGoldWeight) e.jewelryGoldWeight="Gold weight in jewellery is required";
  ...
  const handleSubmit = async (e:React.FormEvent) => {
    ...
    const app: GoldLoanApplication = {
      ...
      typeOfLoan:form.typeOfLoan===OTHER_OPTION?form.typeOfLoanOther:form.typeOfLoan,
      goldCarats:form.goldCarats===OTHER_OPTION?form.goldCaratsOther:form.goldCarats,
      goldWeight:form.typeOfLoan==="Jewellery"?form.jewelryGoldWeight:form.goldWeight,
      jewelryGoldWeight:form.typeOfLoan==="Jewellery"?form.jewelryGoldWeight:undefined,
      jewelryStoneWeight:form.typeOfLoan==="Jewellery"?form.jewelryStoneWeight:undefined,
      jewelryOtherMaterials:form.typeOfLoan==="Jewellery"&&form.jewelryOtherMaterials.length>0?form.jewelryOtherMaterials:undefined,
    ...
    <form className="max-w-4xl mx-auto" onSubmit={handleSubmit} noValidate>
      <div className="mb-6">
        <h1 className="text-xl font-bold" style={{color:C.dark}}>
          Unlock the best Gold Loan offers suitable for your needs from 43+ lenders
        </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div id="loanAmount"><FieldLabel label="Required Loan Amount" required/>
            <TextField type="number" value={form.loanAmount===0?"":String(form.loanAmount)}
              onChange={v=>set("loanAmount",Math.max(0,parseInt(v)||0))} placeholder="e.g. 300000" err={errors.loanAmount}/>
          </div>
          <TenureYearsField
            ...
            options={masters.goldLoanTenureYears} err={errors.loanTenureYears}
          />
          <SelectWithOther
            id="typeOfLoan" label="Type of Loan" required
            value={form.typeOfLoan} onChange={setTypeOfLoan}
            options={masters.goldLoanTypes} err={errors.typeOfLoan}
            otherId="typeOfLoanOther" otherLabel="Mention Type of Loan"
            otherValue={form.typeOfLoanOther} onOtherChange={v=>set("typeOfLoanOther",v)}
            otherPlaceholder="Enter type of loan" otherErr={errors.typeOfLoanOther}
          />
          <SelectWithOther
            id="goldCarats" label="Gold Karat" required
            value={form.goldCarats} onChange={setGoldCarats}
            options={goldCaratsOptions} placeholder={form.typeOfLoan?"Select":"Select type of loan first"}
            disabled={!form.typeOfLoan} err={errors.goldCarats}
            otherId="goldCaratsOther" otherLabel="Mention Gold Karat"
            otherValue={form.goldCaratsOther} onOtherChange={v=>set("goldCaratsOther",v)}
            otherPlaceholder="Enter gold Karat" otherErr={errors.goldCaratsOther}
          />
          {form.typeOfLoan!=="Jewellery"&&(
            <div id="goldWeight"><FieldLabel label="Gold Weight (Approx)" required/>
              <TextField type="number" value={form.goldWeight===0?"":String(form.goldWeight)}
                onChange={v=>set("goldWeight",Math.max(0,parseFloat(v)||0))} placeholder="e.g. 20 (In Grams)" err={errors.goldWeight}/>
            </div>
          )}
          {form.typeOfLoan==="Jewellery"&&(<>
            <div className="md:col-span-2">
              <h3 className="text-sm font-bold mt-2" style={{color:C.dark}}>Total Weight of Jewellery (In Grams)</h3>
            </div>
            <div id="jewelryGoldWeight"><FieldLabel label="Gold Weight in Jewellery" required/>
              <TextField type="number" value={form.jewelryGoldWeight===0?"":String(form.jewelryGoldWeight)}
                onChange={v=>set("jewelryGoldWeight",Math.max(0,parseFloat(v)||0))} placeholder="e.g. 25" err={errors.jewelryGoldWeight}/>
            </div>
            <div id="jewelryStoneWeight"><FieldLabel label="Stone Weight in Jewellery"/>
              <TextField type="number" value={form.jewelryStoneWeight===0?"":String(form.jewelryStoneWeight)}
                onChange={v=>set("jewelryStoneWeight",Math.max(0,parseFloat(v)||0))} placeholder="e.g. 2 (enter 0 if none)"/>
            </div>
            <div className="md:col-span-2" id="jewelryOtherMaterials">
              <FieldLabel label="Other Material Weight in Jewellery"/>
              <div className="grid grid-cols-1 md:grid-cols-[1fr_1fr_auto] gap-3 items-start">
                <TextField value={form.jewelryOtherMaterialName}
                  onChange={v=>set("jewelryOtherMaterialName",v)} placeholder="Material name e.g. Diamond"/>
                <TextField type="number" value={form.jewelryOtherMaterialWeight===0?"":String(form.jewelryOtherMaterialWeight)}
                  onChange={v=>set("jewelryOtherMaterialWeight",Math.max(0,parseFloat(v)||0))} placeholder="Weight in grams e.g. 2"/>
                <button type="button" onClick={addJewelryOtherMaterial}
                  className="px-5 py-2.5 rounded-xl text-sm font-bold text-white shrink-0 transition-all hover:opacity-90 w-full md:w-auto"
                  style={{background:C.teal}}>
                  Add
                </button>
              </div>
              {form.jewelryOtherMaterials.length>0 && (
          ...
          <div id="collateralPropertyMarketValue"><FieldLabel label="Current Gold Market Value (Approx)" required/>
            <TextField type="number" value={form.collateralPropertyMarketValue===0?"":String(form.collateralPropertyMarketValue)}
              onChange={v=>set("collateralPropertyMarketValue",Math.max(0,parseInt(v)||0))} placeholder="e.g. 500000" err={errors.collateralPropertyMarketValue}/>
          </div>
        </div>
      </FormCard>
      ...
      <FormCard title="Income Details" subtitle="Tell us about your employment and income">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="md:col-span-2" id="employmentType"><FieldLabel label="Employment Type" required/>
            <SelectField value={form.employmentType} onChange={setEmploymentType} options={masters.goldLoanEmploymentTypes} placeholder="Select" err={errors.employmentType}/>
          </div>
            ...
            <div id="gstNumber"><FieldLabel label="GST No (if available)"/>
              ...
              <div id="businessEstablishedDate"><FieldLabel label="Date Of Business Establishment" required/>
                <DateField value={form.businessEstablishedDate} onChange={v=>set("businessEstablishedDate",v)}
                  ...
                  portalId="goldloan-business-established-datepicker-portal"/>
              </div>
