  ...
  if (!isSubmitted) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-sm" style={{color:C.gray}}>Submit your education loan application to track its status here.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm" style={{border:`1px solid ${C.navy}18`}}>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                ["Field of Study",submittedApp?.fieldOfStudy??"—"],
              ].map(([l,v])=>(
          ...
          {/* Course details */}
          <div className="bg-white rounded-2xl p-6 shadow-sm" style={{border:`1px solid ${C.teal}18`}}>
            <p className="text-xs font-bold uppercase tracking-wide mb-4" style={{color:C.teal}}>Course Details</p>
            <div className="space-y-3">
              {[
                ["Course",      submittedApp?.courseName??"—"],
                ["University",  submittedApp?.university??"—"],
                ["Institute",   submittedApp?.instituteName??"—"],
                ["Enrollment",  submittedApp?.enrollmentStatus??"—"],
                ["Duration",    `${submittedApp?.courseDuration??0} years`],
                ["Course Cost", `₹${(submittedApp?.educationCost??0).toLocaleString("en-IN")} Lakhs`],
              ].map(([l,v])=>(
          ...
          {/* Student details */}
          <div className="bg-white rounded-2xl p-6 shadow-sm" style={{border:`1px solid ${C.teal}18`}}>
            <p className="text-xs font-bold uppercase tracking-wide mb-4" style={{color:C.teal}}>Student Details</p>
            <div className="space-y-3">
              {[
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm" style={{border:`1px solid ${C.navy}18`}}>
            <p className="text-xs font-bold uppercase tracking-wide mb-4" style={{color:C.navy}}>Co-Applicant Details</p>
            <div className="space-y-3">
              {[
                ["Relationship", submittedApp?.parentRelationship??"—"],
                ["Name",         submittedApp?.parentFullName??"—"],
                ["Mobile",       `+91 ${submittedApp?.parentMobile??"—"}`],
                ["Email",        submittedApp?.parentEmail??"—"],
                ["City",         `${submittedApp?.parentCity??""}, ${submittedApp?.parentState??""}`],
                ["PAN",          submittedApp?.parentPanNumber??"—"],
              ].map(([l,v])=>(
                <div key={l} className="flex items-center justify-between gap-4 py-2 border-b last:border-0"
                  style={{borderColor:"#f1f5f9"}}>
                  <span className="text-xs font-semibold uppercase tracking-wide shrink-0" style={{color:C.gray}}>{l}</span>
