...
interface FormData {
  ...
  parentState:string; parentCity:string; parentPincode:string; parentPincodeOther:string;
...
const ApplicationForm = ({userName="",userEmail="",onSubmit}:ApplicationFormProps) => {
  ...
  const [form, setForm] = useState<FormData>({
    ...
    parentState:"", parentCity:"", parentPincode:"", parentPincodeOther:"",
  ...
  const parentPincodeOptions = useMemo(
    () => form.parentState && form.parentCity ? masters.pincodesByLocation[pincodeLocationKey(form.parentState, form.parentCity)] ?? [] : [],
    [form.parentState, form.parentCity, masters.pincodesByLocation]
  );
  ...
  const computeErrors = (draft:FormData = form) => {
    ...
    else if(draft.parentPincode===OTHER_OPTION){
      if(!draft.parentPincodeOther.trim()) e.parentPincodeOther="Please mention pincode";
      else if(!/^\d{6}$/.test(draft.parentPincodeOther)) e.parentPincodeOther="Enter valid 6-digit pincode";
    }
  ...
  const handleSubmit = async (e:React.FormEvent) => {
    ...
    const app: EducationLoanApplication = {
      ...
      parentPincode:form.parentPincode===OTHER_OPTION?form.parentPincodeOther:form.parentPincode,
      ...
      <FormCard title="Personal Details (Parent)" subtitle="Co-applicant details as per official documents">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <PincodeSelectField
            ...
            options={parentPincodeOptions} cityReady={!!form.parentCity}
            value={form.parentPincode} onChange={v=>set("parentPincode",v)} err={errors.parentPincode}
            otherId="parentPincodeOther" otherValue={form.parentPincodeOther}
            onOtherChange={v=>set("parentPincodeOther",v)} otherErr={errors.parentPincodeOther}
          />
          <SelectWithOther
