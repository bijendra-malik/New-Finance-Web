...
interface ApplicationFormProps {
  userName?: string;
  userEmail?: string;
  onSubmit?: (id: string, app: ODCCLimitApplication) => void;
}
interface FormData {
  fullName:string; mobile:string; email:string; dob:string; panNumber:string;
...
const ApplicationForm = ({userName="",userEmail="",onSubmit}:ApplicationFormProps) => {
  ...
  const [submittedApp, setSubmittedApp] = useState<ODCCLimitApplication|null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    getEmploymentTypesFor("od-cc-limit", "odCcEmploymentTypes")
  ...
  const employmentTypeOptions = useMemo(
    () => (employmentTypesState.length > 0 ? employmentTypesState : [...masters.odCcEmploymentTypes]),
    [employmentTypesState, masters.odCcEmploymentTypes]
  );
...
// ── Shared validation constants (used by computeErrors below) ──
  ...
  const computeErrors = (draft:FormData = form) => {
    ...
    else if(draft.loanTenureYears<3||draft.loanTenureYears>40) e.loanTenureYears="Tenure must be between 3 and 40 years";

    if(!draft.collateralPropertyType) e.collateralPropertyType="Please select what you wish to take the limit against";
    ...
    if(draft.collateralPropertyType!==UNSECURED){
    ...
    else if(!/^[A-Z]{5}[0-9]{4}[A-Z]$/.test(draft.panNumber.toUpperCase())) e.panNumber="Invalid PAN format. Example: ABCDE1234F";
  ...
  const handleSubmit = async (e:React.FormEvent) => {
    ...
    const app: ODCCLimitApplication = {
      _id: `ODCC${Date.now()}`,
    ...
    <form className="max-w-4xl mx-auto" onSubmit={handleSubmit} noValidate>
      ...
      <div className="mb-6">
        <h1 className="text-xl font-bold" style={{color:C.dark}}>
          Unlock the best OD CC Limit offers suitable for your needs from 43+ lenders
        </h1>
        <p className="text-xs mt-1.5" style={{color:C.gray}}>Fields with asterisk mark (*) are mandatory</p>
      </div>

      {/* ── LIMIT REQUIREMENTS ────────────────────────────────────────── */}
      <FormCard title="Limit Requirements" subtitle="How much do you need and for how long?">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div id="loanAmount"><FieldLabel label="Required OD / CC Limit" required/>
            <TextField type="number" value={form.loanAmount===0?"":String(form.loanAmount)}
              onChange={v=>set("loanAmount",Math.max(0,parseInt(v)||0))} placeholder="e.g. 2000000" err={errors.loanAmount}/>
          </div>
          <div id="loanTenureYears"><FieldLabel label="Required Renewal Tenure (in years)" required/>
            <TextField type="number" value={form.loanTenureYears===0?"":String(form.loanTenureYears)}
              onChange={v=>set("loanTenureYears",Math.max(0,parseInt(v)||0))} placeholder="e.g. 5 (between 3-40 years)" err={errors.loanTenureYears}/>
          </div>
          <SelectWithOther
            id="collateralPropertyType" label="Wish To Take OD/CC Limit Against" required
            ...
            }} options={masters.odCcLimitAgainstTypes} err={errors.collateralPropertyType}
          ...
          />
          {form.collateralPropertyType!==UNSECURED&&(<>
            ...
            <div id="gstNumber"><FieldLabel label="GST No (if available)"/>
              ...
              <div id="businessEstablishedDate"><FieldLabel label="Date Of Business Establishment" required/>
                <DateField value={form.businessEstablishedDate} onChange={v=>set("businessEstablishedDate",v)}
                  ...
                  portalId="odcc-business-established-datepicker-portal"/>
              </div>
